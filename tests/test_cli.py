...
class TestMissingApiKey:
    """未配置 API Key 时应给出清晰错误"""

    def test_chat_missing_key(self):
        runner = CliRunner()
        result = runner.invoke(main, ["chat", "--message", "hello"])
        assert result.exit_code != 0
        assert "DASHSCOPE_API_KEY" in result.output or "DASHSCOPE_API_KEY" in (result.exception or "")
...
class TestChatCommand:
    ...
    def test_chat_sync(self, mock_get_client):
    ...
    def test_chat_with_system(self, mock_get_client):
        ...
        mock_choice = MagicMock()
        mock_choice.message.content = "I am a translator"
        mock_choice.message.role = "assistant"
        mock_choice.finish_reason = "stop"
